#' @title 'Load EQAO School csv'
#'
#' @description
#'   Loads all EQAO School files for an assessment year
#'   into a single dataframe
#'
#' @param grade vector in the form 3, 6, 9 or 10
#' @param year vector in the form ####
#' @param bident vector in the form #####
#' @param board vector in the form "DDSB"
...
#'
#' @examples
#' EQAOschool(3, 2017, 66060, "DDSB", "C:/Data/")
#'


EQAOschool <- function(grade, year,bident,board,datadir) {
  datadir.txt <- ifelse(datadir == "v",
                        "V:/Programs/Accountability & Assessment/AA_MasterData/",
                        ifelse(datadir == "c",
                               "C:/Data-Local/", datadir))

  ifelse(grade == 10, {
    OSSLT.1 <- readr::read_csv(paste0(datadir.txt,"G10_",
                                      year,
                                      "_FTE_B01",
                                      bident,
                                      "_1.csv"))
